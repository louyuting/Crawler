...
import com.crawler.Dao.ZhiHuDAO;
import com.crawler.entity.Page;
import com.crawler.entity.User;
import com.crawler.httpclient.HttpRequestUtil;
import com.crawler.parser.zhihu.ZhiHuUserFollowingPageParser;
import com.crawler.parser.zhihu.ZhiHuUserInfoPageParser;
import com.crawler.utils.DateUtil;
import com.crawler.utils.LogUtil;
import com.crawler.utils.StringUtil;
...
public class Crawler {
	 ...
	 * @param seed 种子的URL数组
	 */
    private void initCrawlerSeeds(String seed) {
        LinkQueue.addUnvisitedUri(seed);
    }
	...
	public boolean crawling(String seeds) {
		...
		while((!LinkQueue.unVisitedUriIsEmpty()) && (LinkQueue.getVisitedUriNum() < this.MAX_URI_NUM)){
            ...
            Page page = HttpRequestUtil.get(visitUri);
            //获取的响应界面的信息
			...
			*//** 2.4 根据实体body 对HTML进行解析，返回当前uri中的未爬取的uri集合 *//*
            ...
            /** 把刚才已访问的uri添加到已访问的集合中  */
            LinkQueue.addVisitedUri(visitUri);
            List<String> links = ZhiHuUserFollowingPageParser.getInstance().parse(page);
			/** 2.5 把links添加到未访问uri队列中 */
			for (String link : links) {
				//这里可以把解析出来的uri进行一些条件限制：必须以这个开头http://www.ugirls.com/ 的链接才继续爬取
				if(StringUtil.notNullAndSpace(link)){
					LinkQueue.addUnvisitedUri(link);
				}
			}
            User user = ZhiHuUserInfoPageParser.getInstance().parse(page);
            //插入到数据库
            ZhiHuDAO.insertUser(user);
	...
	public boolean crawling(int threadId) {
		...
		while((!LinkQueue.unVisitedUriIsEmpty()) && (LinkQueue.getVisitedUriNum() < this.MAX_URI_NUM)){
			...
			Page body = HttpRequestUtil.get(visitUri);
...
*/
		...
		/*	Set<String> links = JsoupUtil.ParserHtmlGetUri(body);*/
			...
			/*for (String link : links) {
				//这里可以把解析出来的uri进行一些条件限制：必须以这个开头http://www.ugirls.com/ 的链接才继续爬取
				if(StringUtil.notNullAndSpace(link) && (link.startsWith("http://www.ugirls.com/"))){
					LinkQueue.addUnvisitedUri(link);
				}
			}*/
